            ...
            <div class="shadow sm:rounded-md sm:overflow-hidden">
                ...
                <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center">
                            ...
                            <span v-else class="flex items-center justify-center h-12 w-12 rounded-full overflow-hidden bg-gary-100">
                            ...
                            <button type="button" class="ml-5 bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                <input type="file" @change="onImageChoose" class="absolute opacity-0 cursor-pointer" />
                                Change
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" v-model="model.title" autocomplete="title" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                    </div>
                    <!-- /Title -->
                    ...
                    <div>
                        ...
                        <input type="date" name="expire_date" id="expire_date" v-model="model.expire_date" autocomplete="expire_date" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                    </div>
                    <!-- /Expire Date -->
                ...
                <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                    ...
                    <div v-for="(question, index) in model.questions" :key="question.id">
                        <QuestionEditor :question="question" :index="index" @changeQuestion="changeQuestion" @addQuestion="addQuestion" @deleteQuestion="deleteQuestion"/>
                    </div>
                </div>
...
function saveSurvey() {
    store.dispatch("saveSurvey", model.value).then(
        ({data}) => {
            store.commit("notify", {
                type: 'Success', message: 'Survey was successfully updated.'
            });
