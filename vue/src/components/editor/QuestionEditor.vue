    ...
    <span>
        ...
        <div class="grid gap-3 grid-cols-12">
            ...
            <div class="mt-3 col-span-3">
                ...
                <select id="question_type" name="question_type" v-model="model.type" @change="typeChange" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
        ...
        <!-- Data -->
            <div>
                <div v-if="shouldHaveOption()" class="mt-2">
                    <h4 class="text-sm font-semiblod mb-1 flex justify-between items-center">
                        Options

                        <!-- Add new Option button -->
                        <button type="button" @click="addOption()" class="flex items-center text-sm py-1 px-4 mr-2 rounded-sm text-white bg-gray-600 hover:bg-gray-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 20 20" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                            </svg>
                            &nbsp;Add Option
                    ...
                    <div v-for="(option, index) in model.data.options" :key="option.uuid" class="flex items-center mb-1">
                        ...
                        <input type="text" name="question_text" v-model="option.text" @change="dataChange" class="w-full py-1 px-2 text-xm border border-gray-300 focus:border-indigo-500 rounded-sm" />

                        <!-- Delete Option -->
                        <button type="button" @click="deleteOption(option)" 
                            class="h-6 w-6 rounded-full flex items-center justify-center border border-transparent
